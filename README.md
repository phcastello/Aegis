...
Chat uses `AEGIS_CHAT_MODEL=gpt-5.6-luna`. Async conversation titles use `AEGIS_TITLE_MODEL=gpt-5-nano`, `AEGIS_TITLE_REASONING_EFFORT=minimal`, `OPENAI_API_KEY`, and `AEGIS_TITLE_MAX_OUTPUT_TOKENS=64`; no local model is required. These title settings are configurable for a future model change. `AEGIS_OPENAI_BASE_URL=https://api.openai.com` and `AEGIS_MAX_OUTPUT_TOKENS=4000` apply to chat. The model receives the Gmail tools with automatic selection. The backend still validates arguments, pending actions, confirmation, and effects.

The Responses API request puts stable identity in a `developer` input text block with an explicit cache breakpoint. Runtime context, role based history, and the current message follow it. Cache policy is `explicit`, so dynamic suffixes do not create cache writes. The stable prefix may be too short for caching until tools and context reach the 1,024 token minimum; no padding is added. OpenTelemetry meter `Aegis` exposes input, cached input, cache write, and output token counters, plus model/tool calls, tool iterations, and turn duration. See the [OpenAI prompt caching documentation](https://developers.openai.com/api/docs/guides/prompt-caching).

Run manual tool intent evaluation with `OPENAI_API_KEY=... python3 scripts/eval_tool_intent.py`. It is intentionally excluded from credential free CI.
